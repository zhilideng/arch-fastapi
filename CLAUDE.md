...
- **Redis 异步缓存层已实现**：`app/core/redis.py`（redis.asyncio 进程级单例，`get_redis()` 惰性创建复用连接池，`init_redis(settings.redis)` / `close_redis()` 接入 factory.py lifespan）、`app/repositories/cache.py`（cache-aside 访问封装：`cache_get/cache_set/cache_delete` + JSON 序列化 + TTL、`get_or_set` 工厂模式）、配置段 `RedisSettings`（url/db/max_connections/decode_responses/encoding）、lifespan 启动/关闭已接入、`tests/test_redis.py`（15 个测试，用 fakeredis 模拟，全过）。
- **数据层建设中**：`app/core/database.py`（SQLAlchemy 异步封装）、`app/repositories/base.py`（Repository 基类）、`app/repositories/`（DAO 层：PostgreSQL / Milvus / ES 访问封装）——由其他 agent 负责实现。
- **测试**：`tests/` 共 **67 个测试，65 passed / 2 failed**——新增 `test_redis.py`(15，全过)、`test_config.py`(7)、`test_app.py`(3)、`test_api_response.py`(5)、`test_exceptions.py`(12)、`test_logging.py`(4)、`test_http_client.py`(13)、`test_cors_middleware.py`(5)、`test_routes.py`(2)、`test_request_id_middleware.py`(3)。注意：`test_config.py` 的 `test_app_settings_defaults` / `test_settings_loads_dev_yaml_by_default` 为 **pre-existing 失败**（`dev.yaml` port 由 8002 改 8003 漏同步测试；`settings.py` 的 `name` 默认值残留 `arch-fastapi111`），与本次无关，经决策暂不修复。
...
**下一步**：横向支撑层与数据层待补——`core/llm/`（统一 LLM 抽象，屏蔽厂商差异）、`middleware/`（CORS/RequestID 已完成且写死，将来生产收敛来源需改回配置驱动；剩 JWT / 限流）、`repositories/`（PostgreSQL / Milvus / ES 访问封装）、`tasks/`（Celery / Arq 异步任务）。各层均经 `startup.py` 注册、经 `core/config.py` 取配置。
