...
- **数据层建设中**：`app/core/database.py`（SQLAlchemy 异步封装）、`app/core/redis.py`（Redis 异步客户端）、`app/repositories/base.py`（Repository 基类）、`app/repositories/cache.py`（缓存抽象）——由其他 agent 负责实现。
