...
**下一步**：横向支撑层与数据层待补——`core/llm/`（统一 LLM 抽象，屏蔽厂商差异）、`middleware/`（CORS 已完成且写死全放行，将来生产收敛来源需改回配置驱动；HTTP 客户端参数同写死，将来需按环境调优再改配置驱动；剩 JWT / TraceId / 限流；另 `utils/http_client.close_client()` 待接入 factory lifespan shutdown）、`repositories/`（PostgreSQL / Redis / Milvus / ES 访问）、`tasks/`（Celery / Arq 异步任务）。各层均经 `startup.py` 注册、经 `core/config.py` 取配置。
